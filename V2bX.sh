...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/StriveMoring/BEToXboard/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/StriveMoring/BEToXboard/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://github.com/StriveMoring/Linux-NetSpeed/raw/master/tcpx.sh)
}

update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/StriveMoring/BEToXboard/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
